...
class PeerGUI(tk.Tk): 
    ...
    def create_session_list_window(self):
        ...
        listbox.bind("<Double-Button-1>", lambda event: self.create_activate_session_window(event))
        ...
        create_torrent = ttk.Button(
            button_frame, text="Create Session", command=self.create_torrent_window)
    ...
    def reload_sessions(self, listbox) -> None:
        ...
        for i, file in enumerate(data):
            listbox.insert(tk.END, file['info_hash'])
    ...
    def create_activate_session_window(self, event : tk.Event) -> None:
        """
        ...
        selected_item = event.widget.get(selected_index).strip()
        ...
        submit_button = ttk.Button(form_frame, text="Login", command=lambda: [self.download(selected_item),login_window.destroy()])
        submit_button.grid(row=2, column=0, columnspan=2, pady=5)
        
        

    def download(self, info_hash : str) -> None:
    ...
    def create_torrent_window(self) -> None:
        ...
        if file_path:
            ...
            with open(torrent_path,'r') as file:
                data = json.load(file)
            self.peer.announce(data['info_hash'], '')
