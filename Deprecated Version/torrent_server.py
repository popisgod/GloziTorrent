...
class TorrentServer(socket.socket):
    ...
    def handle_client_commands(self, client: socket.socket, command:  bytes) -> None:
        '''
    ...
    def disconnect(self, sock: socket.socket) -> None:
        ...
        if self.ID_BY_SOCKET.get(sock,''):
            if self.PEER_INFO.get(self.ID_BY_SOCKET[sock],''):
                del self.PEER_INFO[self.ID_BY_SOCKET[sock]]
            del self.ID_BY_SOCKET[sock]
            
        if self.ID_BY_IP.get(sock.getpeername()[0],''):    
            del self.ID_BY_IP[sock.getpeername()[0]]
        
        if self.PEER_PORT.get(sock,''):
            del self.PEER_PORT[sock]
